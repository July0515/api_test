...
class TestUserManagerCase(unittest.TestCase):
    ...
    def test02_edit_user(self):
        ...
        actual_result = self.user.edit_user(TestUserManagerCase.id01,
                                            UserManagerData().user_case1_data.get('new_user')
                                            ,'123456')
        common_assert(self,actual_result )
        self.assertEqual(UserManagerData().user_case1_data.get('new_user'),
                         actual_result['data']['username'])
    ...
    def test03_search_user(self):
        ...
        common_assert(self,actual_result )
        self.assertEqual(UserManagerData().user_case1_data.get('new_user'),
                         actual_result['data']['list'][0]['username'])
